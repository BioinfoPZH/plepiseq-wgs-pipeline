...
# Backward compatibility wrapper for run_nf_pipeline_bacterial.sh
...
# Transform bacterial-specific arguments to unified script format
# This wrapper maps the old parameter names to the new unified script format
UNIFIED_ARGS=()
UNIFIED_ARGS+=("--pipeline_type" "bacterial")

# Process all command-line arguments
while [[ $# -gt 0 ]]; do
    case "$1" in
        --main_image)
            UNIFIED_ARGS+=("--bacterial_main_image" "$2")
            shift 2
            ;;
        --prokka_image)
            UNIFIED_ARGS+=("--bacterial_prokka_image" "$2")
            shift 2
            ;;
        *)
